 ...
 */
class WPB_WRPS_Review_Notice
{

    // Set to true during development to always show the notice regardless of timing.
    const DEV_MODE = false;

    const INSTALL_DATE_OPTION = 'wpb_wrps_installed';
    const META_DISMISSED      = 'wpb_wrps_review_dismissed';
    const META_LATER          = 'wpb_wrps_review_later';
    const REVIEW_URL          = 'https://wordpress.org/support/plugin/wpb-woocommerce-related-products-slider/reviews/?rate=5#new-post';
    const NONCE_ACTION        = 'wpb_wrps_review_notice_action';
    ...
    private function render_notice()
    {
        ...
        $rate_url    = esc_url(add_query_arg(array('wpb_wrps_review_action' => 'rate',    '_wpnonce' => $nonce)));
        $later_url   = esc_url(add_query_arg(array('wpb_wrps_review_action' => 'later',   '_wpnonce' => $nonce)));
        $dismiss_url = esc_url(add_query_arg(array('wpb_wrps_review_action' => 'dismiss', '_wpnonce' => $nonce)));
?>
        <div class="wpb-wrps-review-notice notice" style="border-left: 4px solid #f0b429; padding: 0; margin: 15px 0; border-radius: 3px; box-shadow: 0 1px 3px rgba(0,0,0,.06);">
            <div style="display: flex; align-items: center; padding: 14px 18px; gap: 14px;">
                ...
                <div style="flex: 1; min-width: 0;">
                    <p style="margin: 0 0 4px; font-size: 13.5px; font-weight: 600; color: #1d2327;">
                        <?php esc_html_e('Enjoying WPB Related Products Slider for WooCommerce?', 'wpb-wrps'); ?>
                    </p>
                    <p style="margin: 0 0 11px; font-size: 13px; color: #50575e; line-height: 1.55;">
                        <?php esc_html_e("You've been using the plugin for a week — thank you! If it's been helpful, a quick 5-star review would mean the world to us and help others discover it.", 'wpb-wrps'); ?>
                    </p>
                    <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                        <a href="<?php echo $rate_url; ?>"
                            target="_blank"
                            ...
                            &#9733; <?php esc_html_e('You deserve it!', 'wpb-wrps'); ?>
                        </a>
                        ...
                        <a href="<?php echo $dismiss_url; ?>"
                            ...
                            <?php esc_html_e('I Already Did', 'wpb-wrps'); ?>
                        </a>
                    </div>
                </div>
            </div>
...
<?php
    ...
    {
        if (empty($_GET['wpb_wrps_review_action'])) {
            return;
        }

        $nonce  = ! empty($_GET['_wpnonce']) ? sanitize_text_field(wp_unslash($_GET['_wpnonce'])) : '';
        $action = sanitize_text_field(wp_unslash($_GET['wpb_wrps_review_action']));
        ...
        wp_safe_redirect(remove_query_arg(array('wpb_wrps_review_action', '_wpnonce')));
        exit;
    }
}
